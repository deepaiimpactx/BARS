...
def select_target():
    ...
    # Thread(target=run_autoencoder_task, args=(file_path, target)).start()
...
# def run_autoencoder_task(file_path, target):
...
#     }
#     send_to_kafka(results, 'autoencoder_results')
...
def run_lstm_task(X_train, y_train, X_valid, y_valid,file_path, target):
    ...
    def objective_function(x):
        ...
        # Define your metric, for example, accuracy
    ...
    # print("Selected features PSO:", selected_feature_names)
...
def run_dnn_task(file_path, target):
    ...
    # Evaluate the model using the first 10 features as a test
    ...
    def fitness_function(feature_indices):
        ...
        if selected_features.shape[1] == 0:  # If no features are selected, return a large error
            return float('inf')

        try:
            clf = RandomForestClassifier()
            scores = cross_val_score(clf, selected_features, y, cv=3)  # Reduce CV folds to speed up
            mean_score = -scores.mean()  # Negative because PSO minimizes the function
    ...
    # Define the bounds (binary selection for each feature)
    lb = [0] * features.shape[1]
    ub = [1] * features.shape[1]

    # Run PSO with smaller swarm size and iterations
    xopt, fopt = pso(fitness_function, lb, ub, swarmsize=10, maxiter=2)

    # Map the extracted feature indices back to the original dataframe column names
    selected_features_indices = np.where(np.round(xopt) == 1)[0]
    original_columns =feature_names # Drop target column to get feature columns

    # Ensure that selected feature indices are within the original column range
    ...
    # (not needed )dnn_features = dnn_model.predict(X_valid_scaled)
    results = {
